...
//! Basic MCP Edge Gateway implementation with core functionality

use mcp_common::{Config, MCPRequest, MCPResponse, HealthLevel, HealthStatus};
use std::sync::Arc;
...
/// Simple Gateway implementation for Generation 1
...
impl SimpleGateway {
    ...
    pub async fn health_check(&self) -> anyhow::Result<HealthStatus> {
        ...
        let uptime = Utc::now().signed_duration_since(state.started_at).num_seconds() as u64;
        let success_rate = if metrics.successful_requests + metrics.failed_requests > 0 {
            metrics.successful_requests as f64 / (metrics.successful_requests + metrics.failed_requests) as f64
        } else {
            1.0
        ...
        let health_level = if success_rate > 0.95 && state.active_requests < 100 {
            HealthLevel::Healthy
...
/// Simple HTTP server for Generation 1
pub async fn start_simple_server() -> anyhow::Result<()> {
    info!("🚀 Starting Generation 1 MCP Gateway Server on http://0.0.0.0:8080");
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            cleanup_gateway.cleanup_cache().await;
        }
    });
    ...
    tokio::spawn(async move {
        // Simulate some sample requests for demonstration
        ...
        match gateway_clone.process_request(ping_request).await {
        ...
        match gateway_clone.process_request(list_request).await {
            Ok(response) => info!("✅ List models successful"),
        ...
        match gateway_clone.process_request(completion_request).await {
            Ok(response) => info!("✅ Completion successful"),
        ...
        match gateway_clone.process_request(health_request).await {
            Ok(response) => info!("✅ Health check successful: {:?}", response.result),
            Err(e) => error!("❌ Health check failed: {}", e),
        }

        // Display metrics
        let metrics = gateway_clone.get_metrics().await;
        let state = gateway_clone.get_state().await;
    ...
    loop {
        ...
        debug!("Gateway health: {:?}", health.overall_health);
        
        if health.overall_health == HealthLevel::Critical {
            error!("❌ Gateway health is critical, shutting down");
            break;
        }
    }

    info!("🔄 Generation 1 Server shutting down gracefully");
    Ok(())
}
