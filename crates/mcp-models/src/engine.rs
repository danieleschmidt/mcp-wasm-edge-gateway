...
use mcp_common::{Result, Error, MCPRequest, MCPResponse, ComponentHealth, Config, ModelId, HealthLevel};
use std::sync::Arc;
...
impl StandardModelEngine {
    ...
    async fn execute_inference(&self, request: &MCPRequest, model_id: &ModelId) -> Result<serde_json::Value> {
        ...
        let result = match request.method.as_str() {
            "completion" => {
                self.execute_completion(request, model_id).await?
            }
            "embedding" => {
                self.execute_embedding(request, model_id).await?
            }
            "chat" => {
                self.execute_chat(request, model_id).await?
            }
            "summarization" => {
                self.execute_summarization(request, model_id).await?
            }
            _ => {
                return Err(Error::Model(format!("Unsupported method: {}", request.method)));
            }
        };
    ...
    async fn execute_completion(&self, request: &MCPRequest, _model_id: &ModelId) -> Result<serde_json::Value> {
    ...
    async fn execute_embedding(&self, request: &MCPRequest, _model_id: &ModelId) -> Result<serde_json::Value> {
    ...
    async fn execute_chat(&self, request: &MCPRequest, _model_id: &ModelId) -> Result<serde_json::Value> {
        let messages = request.params.get("messages")
    ...
    async fn execute_summarization(&self, request: &MCPRequest, _model_id: &ModelId) -> Result<serde_json::Value> {
        let text = request.params.get("text")
            .and_then(|v| v.as_str())
            .unwrap_or("");
...
impl ModelEngine for StandardModelEngine {
    ...
    async fn load_model(&self, model_id: &ModelId) -> Result<()> {
        ...
        if total_memory_usage + estimated_memory > self.config.models.cache_size_mb {
            ...
            let mut models_by_usage: Vec<_> = models.iter().collect();
            models_by_usage.sort_by_key(|(_, model)| model.last_used);
            
            // Unload least recently used models
            while total_memory_usage + estimated_memory > self.config.models.cache_size_mb {
                if let Some((id, _)) = models_by_usage.first() {
                    let id = id.clone();
                    warn!("Unloading model {} to free memory", id);
                    models.remove(&id);
                    models_by_usage.remove(0);
                } else {
                    break;
                }
            }
    ...
    async fn health_check(&self) -> Result<ComponentHealth> {
        ...
        let total_memory_usage: u32 = models.values().map(|m| m.memory_usage_mb).sum();
        let memory_usage_percent = (total_memory_usage as f32 / self.config.models.cache_size_mb as f32) * 100.0;
        ...
        health_metrics.insert("max_models".to_string(), self.config.models.max_models_in_memory as f32);
        
        let status = if memory_usage_percent > 95.0 || models.len() >= self.config.models.max_models_in_memory as usize {
