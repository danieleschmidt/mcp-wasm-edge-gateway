...
use mcp_common::{Result, Error, ComponentHealth, Config, AggregatedMetrics};
use std::sync::Arc;
use uuid::Uuid;
...
pub async fn create_telemetry_collector(config: Arc<Config>) -> Result<Arc<dyn TelemetryCollector + Send + Sync>> {
