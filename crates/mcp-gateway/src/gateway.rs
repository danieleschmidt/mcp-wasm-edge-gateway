...
use std::sync::Arc;
use mcp_common::{Result, Error, MCPRequest, MCPResponse, Config};
...
impl Gateway {
    ...
    pub async fn process_request(&self, mut request: MCPRequest) -> Result<MCPResponse> {
        ...
        match &result {
            Ok(response) => {
                ...
                self.telemetry.record_request_success(request_id, response).await;
            }
            Err(error) => {
    ...
    async fn process_request_internal(&self, request: MCPRequest) -> Result<MCPResponse> {
        ...
        let response = match routing_decision {
            mcp_common::RoutingDecision::Local { model_id, .. } => {
                self.model_engine.process_request(&request, &model_id).await?
            }
            mcp_common::RoutingDecision::Cloud { endpoint, .. } => {
                self.router.forward_to_cloud(&request, &endpoint).await?
            }
            mcp_common::RoutingDecision::Queue { reason, .. } => {
    ...
    pub async fn health_check(&self) -> Result<mcp_common::HealthStatus> {
        ...
        health_status.components.insert(
            "router".to_string(),
            self.router.health_check().await.unwrap_or_else(|_| {
        ...
        health_status.components.insert(
            "queue".to_string(),
            self.queue.health_check().await.unwrap_or_else(|_| {
                mcp_common::ComponentHealth {
                    status: mcp_common::HealthLevel::Critical,
        ...
        health_status.components.insert(
            "security".to_string(),
            self.security.health_check().await.unwrap_or_else(|_| {
                mcp_common::ComponentHealth {
                    status: mcp_common::HealthLevel::Critical,
        ...
        health_status.components.insert(
            "telemetry".to_string(),
            self.telemetry.health_check().await.unwrap_or_else(|_| {
                mcp_common::ComponentHealth {
                    status: mcp_common::HealthLevel::Critical,
