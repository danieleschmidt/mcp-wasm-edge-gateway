...
use mcp_common::{Result, Error, MCPRequest, MCPResponse, RoutingDecision, ComponentHealth, Config, HealthLevel};
use std::sync::Arc;
...
impl IntelligentRouter {
    ...
    async fn make_routing_decision(&self, request: &MCPRequest) -> Result<RoutingDecision> {
        ...
        match &self.config.router.strategy {
            mcp_common::RoutingStrategy::ComplexityBased => {
                if complexity > self.config.router.local_processing_threshold {
                    ...
                    if self.cloud_client.is_some() && !self.config.router.cloud_endpoints.is_empty() {
                        ...
                        Ok(RoutingDecision::Queue {
                            reason: "High complexity request queued for later processing".to_string(),
                            retry_after_ms: 5000,
                        })
                    }
                } else {
            ...
            mcp_common::RoutingStrategy::ResourceAware => {
            ...
            mcp_common::RoutingStrategy::PerformanceOptimized => {
                ...
                let cloud_avg_latency = if !self.config.router.cloud_endpoints.is_empty() {
                    metrics.cloud_performance.get(&self.config.router.cloud_endpoints[0].url)
                        .unwrap_or(&2000.0)
                } else {
                    &f32::MAX
                };
            ...
            mcp_common::RoutingStrategy::Hybrid { weights } => {
                ...
                let resource_pressure = (metrics.resource_usage.cpu_usage_percent + 
                                       metrics.resource_usage.memory_usage_percent) / 200.0;
                local_score += (1.0 - resource_pressure) * weights.resource_usage;
                cloud_score += resource_pressure * weights.resource_usage;
                
                // Performance factor
                let local_perf = 1.0 / (metrics.local_performance.get("default").unwrap_or(&500.0) / 1000.0);
                let cloud_perf = if !self.config.router.cloud_endpoints.is_empty() {
                    1.0 / (metrics.cloud_performance.get(&self.config.router.cloud_endpoints[0].url)
                ...
                debug!("Routing scores - Local: {:.2}, Cloud: {:.2}", local_score, cloud_score);
                ...
                } else if self.cloud_client.is_some() && !self.config.router.cloud_endpoints.is_empty() {
                    Ok(RoutingDecision::Cloud {
                        endpoint: self.config.router.cloud_endpoints[0].url.clone(),
...
impl Router for IntelligentRouter {
    ...
    async fn update_metrics(&self, metrics: &mcp_common::PerformanceMetrics) -> Result<()> {
        ...
        debug!("Updated router metrics: CPU {:.1}%, Memory {:.1}%", 
               store.resource_usage.cpu_usage_percent,
               store.resource_usage.memory_usage_percent);
    ...
    async fn health_check(&self) -> Result<ComponentHealth> {
        ...
        health_metrics.insert("cpu_usage_percent".to_string(), metrics.resource_usage.cpu_usage_percent);
        health_metrics.insert("memory_usage_percent".to_string(), metrics.resource_usage.memory_usage_percent);
        health_metrics.insert("active_requests".to_string(), metrics.resource_usage.active_requests as f32);
        health_metrics.insert("decision_history_size".to_string(), metrics.request_history.len() as f32);
