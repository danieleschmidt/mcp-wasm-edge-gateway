...
impl CloudClient {
    ...
    pub async fn forward_request(&self, request: &MCPRequest, endpoint: &str) -> Result<MCPResponse> {
        debug!("Forwarding request {} to cloud endpoint: {}", request.id, endpoint);
        
        // Find the endpoint configuration
        let endpoint_config = self.config.router.cloud_endpoints
            .iter()
    ...
    pub async fn test_endpoint(&self, endpoint: &str) -> bool {
        let endpoint_config = match self.config.router.cloud_endpoints
            .iter()
            .find(|e| e.url == endpoint) {
            Some(config) => config,
            None => return false,
