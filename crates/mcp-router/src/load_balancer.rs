...
use mcp_common::{CloudEndpoint, LoadBalancingAlgorithm};
...
impl LoadBalancer {
    pub fn new(algorithm: LoadBalancingAlgorithm, endpoints: Vec<CloudEndpoint>) -> Self {
        let endpoint_health = endpoints.iter()
            .map(|e| (e.url.clone(), EndpointHealth {
                is_healthy: true,
                failure_count: 0,
    ...
    pub async fn select_endpoint(&self) -> Option<&CloudEndpoint> {
        ...
        match self.algorithm {
            LoadBalancingAlgorithm::RoundRobin => {
                let index = self.round_robin_counter.fetch_add(1, Ordering::Relaxed) % self.endpoints.len();
            ...
            LoadBalancingAlgorithm::LeastConnections => {
                ...
                for endpoint in &self.endpoints {
                    if let Some(health_info) = health.get(&endpoint.url) {
                        if health_info.is_healthy && health_info.active_connections < min_connections {
    ...
    pub async fn connection_ended(&self, url: &str) {
        let mut health = self.endpoint_health.write().await;
        if let Some(endpoint_health) = health.get_mut(url) {
            endpoint_health.active_connections = endpoint_health.active_connections.saturating_sub(1);
        }
    }
